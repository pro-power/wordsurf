...
const WordChainGame = () => {
  ...
  useEffect(() => {
    ...
    if (savedHighScore) {
      ...
      // Fetch the leaderboard if no saved high score
      const fetchLeaderboard = async () => {
        try {
          const leaderboard = await leaderboardService.getTopScores();
          if (leaderboard && leaderboard.length > 0) {
            // Get the player with the highest score
            const highestPlayer = leaderboard[0]; // Assuming leaderboard is sorted by score
            setHighScore(highestPlayer);
            localStorage.setItem('wordChainHighScore', JSON.stringify(highestPlayer)); // Save the full object in localStorage
          }
        } catch (error) {
  ...
  //check lowest score on leaderboard
  const checkHighScore = async (finalScore) => {
    try {
      const topScores = await leaderboardService.getTopScores();
      
      if (topScores.length < 5 || finalScore > topScores[topScores.length - 1].score) {
  ...
  // Update the saveHighScore function
const saveHighScore = async () => {
  ...
  try {
    ...
    await leaderboardService.saveScore(scoreData);
  ...
  const submitWord = async () => {
    ...
    try {
      ...
      setError('');
      playSound('submit');
      setTimeout(() => inputRef.current?.focus(), 0);   // Focus even on success
    } catch (err) {
      setError('Error validating word');
      playSound('error');
      setTimeout(() => inputRef.current?.focus(), 0);  // Focus even on error
    }
    setIsLoading(false);
  };
  ...
  // Add an effect to focus the input when the game starts
  useEffect(() => {
    if (gameActive) {
    ...
    // Effect to handle focus after currentWord changes
    useEffect(() => {
    ...
    <Card className="w-full max-w-4xl mx-auto">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                  <Input
                    ...
                    className="flex-1"
                    disabled={isLoading}
                    // Add onBlur to refocus if the input loses focus during game
                    onBlur={() => {
                      if (gameActive) {
                        setTimeout(() => inputRef.current?.focus(), 0);
                      }
      ...
      <DialogContent>
        ...
        <div className="py-4 space-y-4">
          <p>Amazing! You scored {score} points!</p>
          <Input
          ...
          <p className="text-sm text-gray-500">
            Your email will be used to verify your high score and notify you of new achievements.
          </p>
        </div>
        <DialogFooter>
